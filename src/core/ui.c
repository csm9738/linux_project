...
int start_ui(const char* git_log_filepath) {
    ...
    do {
        ...
        if (fp == NULL) {
            endwin();
            printf("Failed to open git log file: %s\n", git_log_filepath);
            return 1;
        }
        ...
        while (1) {
            ...
            switch (ch) {
                ...
                case '\n':
                    if (active_window == 1) {
                        if (right_highlight == 0) { // Preview
                            run_preview(lines[left_highlight]);
                            // After popup, need to refresh screen
                            clear();
                            refresh();
                        } else if (right_highlight == 3) { // Commit
                            should_commit = true;
                            goto end_ui_loop; // Break from inner while
                        }
                    }
                    break;
            }
...
end_ui_loop:
        ...
        delwin(left_win);
        delwin(right_win);
        endwin(); // Requirement 1: Close ncurses
        for (int i = 0; i < num_lines; ++i) free(lines[i]);
        free(lines);

        // --- EXTERNAL ACTION ---
        if (should_commit) {
            system("bash src/modules/commit.sh");
        }
